...
"""
from sqlalchemy import Column, Integer, String, Float, Boolean, ForeignKey, Enum as SQLEnum, Date, Time, DateTime
...
class Table(Base):
    """Table model for hall map."""
    __tablename__ = "tables"
    
    id = Column(Integer, primary_key=True, index=True)
    zone = Column(SQLEnum(Zone), nullable=False)
    seats = Column(Integer, nullable=False)
    x = Column(Float, nullable=False)  # SVG coordinate X
    y = Column(Float, nullable=False)  # SVG coordinate Y
...
class Review(Base):
    ...
    image_url = Column(String, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    is_approved = Column(Boolean, default=False, nullable=False)  # For moderation
